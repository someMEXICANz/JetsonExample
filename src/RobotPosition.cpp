...
bool RobotPosition::initialize(boost::asio::io_service& io_service) 
{
    ...
    {
        std::cerr << "Error: Brain communication has not been established" << std::endl;
        return false;
    }

    auto gps_ports = PortDetector::findGPSPorts(); // Find available GPS ports
    ...
    gps_identified = identifyGPSSensors();
        
    if(gps_identified)
    {
        std::cerr << "Warning: Could not identify GPS sensors" << std::endl;

        if(!start())
        {
            std::cerr << "Failed to start Robot Posistion thread" << std::endl;
            return false;
        }
        else
            std::cerr << "Started Robot Position thread" << std::endl;
    }
    else
        std::cerr << "Could not start Robot Position because GPS devices have not been identified" << std::endl;
...
bool RobotPosition::identifyGPSSensors() 
{
    ...
    if (!offsets_received) 
    {
        ...
        std::cout << "Left GPS offset: (" << left_gps_offset.x << ", " 
                  << left_gps_offset.y <<  ", " << left_gps_offset.heading << ")" << std::endl;
        std::cout << "Right GPS offset: (" << right_gps_offset.x << ", " 
